...
public class ExifDetailsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent == null) {
			...
			imageMenuHandler.saveTemporaryImage((savedImage, ignored) -> loadExifDetails(imageFilename, savedImage));
		}
	 ...
	 */
	protected void loadExifDetails(String imageFilename, @NonNull Uri imageFile) {
		try {
			Timber.i("Loading exif details for image '%s'", imageFile.getPath());

			if (isScopedStorageEnabled()) {
				ContentResolver contentResolver = getApplicationContext().getContentResolver();

				try (InputStream inputStream = contentResolver.openInputStream(imageFile)) {
					ExifInterface exifInterface = new ExifInterface(inputStream);

					addDetailIfPresent(imageFilename, extractImageSize(imageFile, exifInterface), R.drawable.ic_photo_white_24dp);
			...
			Timber.e(e, "Unable to extract EXIF information from file '%s'", imageFile.getPath());
		}
	}
	...
	protected String extractImageSize(Uri imageFile, ExifInterface exifInterface) {
		...
		long imageFileSizeInBytes = image.length();
		Timber.d("Image length = %d", imageFileSizeInBytes);

		String imageSize;
		if (imageFileSizeInBytes < 1024 * 1024) {
			imageSize = Math.round(10.0 * imageFileSizeInBytes / 1024.0) / 10.0 + " Ko";
		} else {
			imageSize = Math.round(10.0 * imageFileSizeInBytes / 1024 * 1024.0) / 10.0 + " Mo";
		}
