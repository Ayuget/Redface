...
public class DiberieHostService implements ImageHostingService {
	...
	@Override
	public Observable<HostedImage> hostFromLocalImage(ByteString localImage) {
		return Observable
				.defer(() -> {
					try {
	...
	private HostedImage uploadToDiberie(ByteString image) throws IOException {
		...
		try (Response response = uploadClient.newCall(request).execute()) {
			...
			if (!response.isSuccessful()) {
				throw new IOException("Unexpected code " + response + " body=" + respBodyStr);
			}

			if (respBodyStr == null) {
				throw new IOException("Got a null response body from Diberie hosting service");
			}
